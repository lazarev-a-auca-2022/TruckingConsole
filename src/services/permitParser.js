...
async function convertPdfToImages(pdfPath) {
  try {
    ...
    // Ghostscript command: -r200 for 200 DPI (good quality for OCR)
    const gsCommand = `gs -dNOPAUSE -dBATCH -sDEVICE=png16m -r200 -sOutputFile="${outputPattern}" "${pdfPath}"`;
    
    logger.info(`Executing: ${gsCommand}`);
    ...
    if (pngFiles.length === 0) {
      throw new Error('No PNG files were generated by Ghostscript');
    }
    
    logger.info(`✅ Converted PDF to ${pngFiles.length} images using Ghostscript`);
